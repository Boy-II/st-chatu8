...
const filesToObfuscate = ['index.js', 'chatu 16.6.js'];
...
const obfuscationOptions = {
    ...
    // 启用 sourceMap 以便调试
    sourceMap: true,
    sourceMapBaseUrl: 'http://localhost:8000/', // 根据你的本地服务器地址修改
...
allFiles.forEach(filePath => {
    ...
    if (obfuscationResult.getSourceMap()) {
        const sourceMapPath = path.join(outputDir, `${filePath}.map`);
        fs.writeFileSync(sourceMapPath, obfuscationResult.getSourceMap());
        console.log(`Source map 已生成: ${sourceMapPath}`);
    }
});
